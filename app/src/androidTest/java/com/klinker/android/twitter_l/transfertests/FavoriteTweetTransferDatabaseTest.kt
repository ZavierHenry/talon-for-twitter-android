...
import com.klinker.android.twitter_l.mockentities.transferentities.MockTransferFavoriteTweet
...
class FavoriteTweetTransferDatabaseTest {
    ...
    fun basicFavoriteTweetsTransfer() {
        ...
        val mockTransferFavoriteTweet = MockTransferFavoriteTweet(1, tweet = tweet)

        val oldId = database.insertIntoSQLiteDatabase(mockTransferFavoriteTweet)
        assertThat("Favorite tweet did not enter into initial database", oldId, not(equalTo(-1L)))
        ...
        val mockFavoriteTweet = database.queryFromTalonDatabase("SELECT * FROM favorite_tweets LIMIT 1")!!.use { cursor ->
            cursor.moveToFirst()
            MockFavoriteTweet(cursor)
        }

        val expected = MockFavoriteTweet(1, tweet = tweet.copy(liked = true), id = mockFavoriteTweet.id)
        assertThat("Entities are not equal", mockFavoriteTweet, matchesMockEntity(expected))
